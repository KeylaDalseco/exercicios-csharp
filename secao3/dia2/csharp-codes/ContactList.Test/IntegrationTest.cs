...
public class IntegrationTest: IClassFixture<WebApplicationFactory<Program>>
    {
      ...
      [Theory(DisplayName = "Testando a rota /GET Person")] // descritivo pro teste
      [InlineData("/person")] // valor para o parametro de entrada
      public async Task TestGetPerson(string url)
      {

          // ARRANGE - MOCKAR A FUNÇÃO COMOS DADOS
          ...
          mockService.Setup(s => s.getPersonList()).Returns(personMoq); // cria mock do get


          // ACT - EXECUTAR A AÇÃO

          var response = await _clientTest.GetAsync(url); // pegou o valor do retorno
          var responseString = await response.Content.ReadAsStringAsync(); // corpo de resposta convertida em string, para depois voltar a ser json.
      // a bibli do Newtonsoft le a string como array de json e converte cada objeto de json para um do tipo Person, gerando um array de Person.
          Person[] jsonResponse = JsonConvert.DeserializeObject<Person[]>(responseString)!; // retorno da requisição


          // ASSERT - VALIDAR O ESPERADO
